        ...
        if SAVE_TO_DISK:
            ...
            if TILE_SIZE_X == tile_len_x and TILE_SIZE_Y == tile_len_y:
                Image.fromarray(tile_data).save(
                    OUTPUT_DIR / f"tile_{x_pos:06d}_{y_pos:06d}.jpg", quality=95
                )
            else:
                ...
                padded = np.full(
                    (height + bottom_padding, width + right_padding, channel), 255, dtype=tile_data.dtype)
                padded[bottom_padding:, :width, :] = tile_data
                Image.fromarray(padded).save(
                    OUTPUT_DIR / f"tile_{x_pos:06d}_{y_pos:06d}.jpg", quality=95)

        api_call(x_pos, y_pos)
        return {"x": x_pos, "y": y_pos, "tile": tile_data}
